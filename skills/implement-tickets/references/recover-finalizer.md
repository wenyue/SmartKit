...
owns no finalizer effect or classification and never authorizes a replacement invocation. Enter
with the original invocation identity when the host exposes one, exact frozen input, expected
pre-state, reviewed base/`HEAD`/tree, dependency contract, retained handoffs, and all observed
effects.
...
to [`complete-run.md`](complete-run.md)'s sole finalizer-result route. Never invent a recovery or
