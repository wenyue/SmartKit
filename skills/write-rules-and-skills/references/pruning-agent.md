...
Look for stale or duplicated meaning, environment facts cached without need, misplaced
branch-specific material, and wording or structure that can be removed or compressed. Challenge
explicit instructions that merely restate evidenced Agent or host defaults, repeat a reliably
loaded broader owner, or follow uniquely from the trigger, inputs, steps, or adjacent context.
...
Treat an instruction as removable only when deleting it leaves every representative action,
choice, authority, safety boundary, and completion, stop, or failure exit unchanged. Support a
claimed default with governing host evidence or an applicable Behavior Control.

## Return and recheck

Return `PASS` or `FAIL`. `PASS` means no supported semantic-preserving reduction remains; it does
not require a smaller artifact. Every `FAIL` finding names its evidence, candidate location,
proposed reduction, unchanged behavior, preserved obligations, and one shared finding class.
