...
migration state. Investigate its meaning and supported recovery before selecting a repair.

## Establish The Data Contract

Identify the owner, the data's role, recovery authority, acceptable loss, and the state required
after repair. Gather evidence using the relevant questions:
...
The evidence may support reconstruction, repair, or accepted discard. Exact reconstruction is
unnecessary when the accepted contract establishes a valid state without it. A path, format, or
corruption finding alone establishes no loss authority. Carry unresolved material decisions back
to the workflow's readiness step.

## Design And Check The Repair
...
For example, suppose the owning parser proves that saved sign-in state is structurally invalid,
and the accepted data contract permits discarding it because sign-in reconstructs that state.
Removing the invalid bytes can prevent identical failures on later startup reads. The operation
