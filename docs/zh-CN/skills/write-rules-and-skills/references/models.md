...
在产生任何编排影响、启动角色、加锁或改变 Candidate 前，完成此 Gate。
...
已接受的 Issue 或 Spec 可以提供这些值。具有唯一证据支持的本地修复也可以补齐它们。否则，
返回`ALIGNMENT_REQUIRED`，并为每个未解决选择给出证据、决策 Owner 和实质后果。不要产生任何
Job 影响。每个实际生效的术语与含义都需要独立、已接受的支持；环境中的词汇表或非规范词汇表
绝不是权威。
