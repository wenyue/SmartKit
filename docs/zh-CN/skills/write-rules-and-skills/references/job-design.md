...
Version、已提供的证据、发现授权、单元、轮次和独立判断边界。
...
- **Candidate：**Owner、模型、写作指引、调用 metadata、准确 Allowlist 与受影响表面、
  依赖、初始 Authoring Scope、读取与网络授权、指纹方法、锁和更新 envelope。
...
分别记录`read`、`write`、`create`、`delete`和`network`；一种模式不授权另一种。Rule 通常只
授权其准确文件。多资源 Skill 只能授权自己的根目录，绝不能授权父`skills/`目录。Candidate
操作应优先使用准确路径；读取与网络发现可以使用范围狭窄的自有来源或能力类别。只有新名称在
冻结时无法得知，才允许在自有资源内创建目录。每次删除都需要准确授权。

在这些授权下，把`role-launch.md`的通用证据选择政策应用于 Author 和 Reviewer。该政策准许的
来源选择属于普通发现，而不是有界更新或逐文件研究路径。

定义每个可由**Context Supplement**填充的必要事实槽位，以及每个符合访问扩展条件的已授权本地
路径类别、外部来源与能力类别和已授权模式。只有无法从当前已授权来源与能力发现该事实，且不会
改变含义、Owner、义务、分支、验证或依赖时，supplement 才能填充一个已声明槽位。访问扩展是缺少
访问权限时的例外处理，必须准确匹配冻结类别内所请求的本地路径与模式，或外部来源、能力与网络
模式。任一有界更新之后，都要恢复同一身份。
...
把`role-launch.md`作为本次运行唯一的固定运行时和通用证据选择政策应用，公共、项目本地或共享
writer 均不得改变它。使用已完成的图、scope、授权、适用性决定和调度，建立全新启动、持久继续
与结束、权威合同投递、有界更新、显式授权、经认证的 Author↔finding-owner 直接通道、规范化
审计，以及覆盖准确 Candidate Allowlist 的一个有界排他锁。
