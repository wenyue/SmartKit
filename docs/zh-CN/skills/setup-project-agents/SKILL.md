...
5. 检查完整的项目直接 Rule 集合带来的始终加载上下文成本。当实质性体量来自仅在可识别条件下适用的分支时，建议应迁移到原生 `rule-<domain>` Skills 的确切分支；不能仅凭文件大小或无条件的基础策略建议迁移。报告来源模式、根目录、指纹和存在时的提交；已启用的宿主；变更与保留路径；外部来源信息；干净检查状态；上下文负担评估；以及任何恢复证据。将项目快照交给维护者审查并提交。
