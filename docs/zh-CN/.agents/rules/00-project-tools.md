...
| `agents/registry.json` 或 `agents/source/` | `<python> scripts/sync_agent_adapters.py` | `<python> scripts/sync_agent_adapters.py --check` | 每个宿主适配器都与 Agent 注册表和源文件一致。 |
...
基线验证包括上述 `VERSION`、Agent 适配器和 MCP 适配器的只读漂移检查，还包括：
...
初始化或校正目标仓库的 `.agents` Rules、Skills、Agents、MCP 声明以及由 Setup 管理的宿主投影时，
